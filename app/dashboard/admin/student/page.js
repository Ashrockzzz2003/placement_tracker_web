...
export default function AllPlacedStudentsScreen() {
    ...
    const [userAccess, setUserAccess] = useState(""); //userAcess not used
    ...
    const [totalStudents, setTotalStudents] = useState(0); //totalStudents not used
    ...
    useEffect(() => {
        if (allPlacedStudentData.length && companyList.length) {
            setAllPlacedStudentDataFiltered(allPlacedStudentData.filter((student) => {
                return (
                    // Even here , is both the null check and 0 check needed??
