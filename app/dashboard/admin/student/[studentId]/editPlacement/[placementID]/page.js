...
export default function NewPlacementScreen() {
    ...
    const [studentId, _setStudentId] = useState(
        student ? student.studentId : "",
    );
    const [studentRollNo, _setStudentRollNo] = useState(
    ...
    useEffect(() => {
        if (!student) {
            console.error(
                "Student data is not available in secure local storage.",
            );
        ...
        setPlacementDate(
            placement.placementDate
                ? placement.placementDate.substring(0, 10)
                : "",
        );
    ...
    */
    ...
    const handleEditPlacement = async (e) => {
        ...
        try {
            ...
            const response = await fetchWithRetry(STUDENT_EDIT_PLACEMENT_URL, {
                method: "POST",
                headers: {
                    "Content-Type": "application/json",
                    Authorization:
                    ...
                    <header className="absolute inset-x-0 top-0 z-50">
                        ...
                        >
                            ...
                            <div className="flex lg:flex lg:flex-1 lg:justify-end">
                                <Link
                                    href={`/dashboard/admin/student/${studentId}`}
