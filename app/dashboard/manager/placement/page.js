                ...
                if (res.status === 200) {
                    res.json().then((data) => {
                        ...
                        data.companyHireData.forEach((data) => {
                            const { companyName: name, totalHires: hires } =
                                data;
            ...
            .then((res) => {
                if (res.status === 200) {
                    res.json().then((data) => {
                        ...
                        const groupedData = data["placementData"].reduce(
                            (acc, obj) => {
                        ...
                        const sortedData = Object.values(groupedData).sort(
                            (a, b) => {
                                return (
                                    new Date(b["placements"][0]["ctc"]) -
                                    new Date(a["placements"][0]["ctc"])
                                );
                            },
                        );
